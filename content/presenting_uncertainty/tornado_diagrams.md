...
Example: [Electricity Generation Costs (2020), BEIS](https://www.gov.uk/government/publications/beis-electricity-generation-costs-2020). The graph presents the change in the levelized cost of combined cycle gas turbine (CCGT) energy (&#163;/MWh) that would result from changes in input parameters. Impacts are shown from both 10% upward or downward movement in central estimates and from predetermined parameters ranges (high/low data range). Orange bars represent an increase in the parameter, blue bar represent a decrease in the parameter.
